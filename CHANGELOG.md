...
## [0.5.0] - 2026-04-11 - Sprint E Backend (POS avancé)

### Added
- **Ouverture/Clôture de caisse (E1)**
  - `POST /api/v1/cash-register-sessions/open` - Ouverture de caisse avec fond initial
  - `POST /api/v1/cash-register-sessions/close` - Clôture de caisse avec calcul de reliquat
  - `GET /api/v1/cash-register-sessions` - Historique paginé des sessions de caisse
  - Génération d'avoir (voucher code) lors d'un reliquat positif à la clôture
- **Vente flexible et filtrage (E2)**
  - `GET /api/v1/sales/products` enrichi avec filtres `code`, `name`, `categoryId` (en plus de `search`)
  - `POST /api/v1/customers/walk-in` pour récupérer/créer le client divers par tenant
  - Politique boutique `allowSaleWithoutStock` pour autoriser la vente hors stock
- **Prix et promotions (E3)**
  - Support des niveaux de prix produit (`BASE`, `LEVEL_1`, `LEVEL_2`, `LEVEL_3`) au checkout
  - `GET /api/v1/sales/promotions/validate` pour valider un code promo
  - Application des remises promo dans les totaux de vente
- **Multi-paiement et impression (E4)**
  - Checkout compatible paiements fractionnés (`payments[]`) avec validation des montants
  - Support `voucherCode` à l'encaissement
  - `GET /api/v1/sales/{saleId}/receipt?format=THERMAL|A4` pour restitution des données d'impression
  - `POST /api/v1/sales/promotions` pour créer un code promo (tenant_admin, shop_admin)
  - `POST /api/v1/cash-register-sessions/remainder-vouchers` pour générer le reliquat manuellement

### Changed
- **Domaine Vente**
  - `Sale` étendu avec `promoCode`, `promoDiscountAmount`, `voucherCode`, `voucherAmount`, `cashRegisterSessionId`
  - Ajout des paiements détaillés par vente (`sale_payments`)
  - Renforcement de l'isolation boutique dans `getSaleById`
- **Domaine Boutique**
  - `Shop` étendu avec `allowSaleWithoutStock`
- **Domaine Produit**
 - **Caisse**
  - Clôture de caisse: suppression de la génération automatique d'avoir/reliquat.
  - Génération d'avoir déplacée vers endpoint dédié avec montant explicite.
 - **Promotions**
  - Scope de promo supporté au niveau boutique (`shop_id`) pour limiter l'usage selon le profil et le contexte boutique.
  - Ajout des colonnes `price_level_1`, `price_level_2`, `price_level_3`

### Database
- **Migration Flyway V10**
  - Nouvelles tables: `cash_register_sessions`, `voucher_codes`, `promotion_codes`, `sale_payments`
  - Nouvelles colonnes sur `sales`, `shops`, `products`
- **Migration Flyway V11**
  - Ajout des tables d'audit Envers: `cash_register_sessions_aud`, `voucher_codes_aud`, `sale_payments_aud`
- **Migration Flyway V13**
  - Ajout du scope boutique sur `promotion_codes` (`shop_id`)
...
- Mise à jour de `contracts/README.md`

### Tests
- Mise à jour des tests unitaires Vente/Boutique
- Ajout de tests unitaires: `CashRegisterSessionServiceTest`, `PromotionServiceTest`, `VoucherCodeServiceTest`
- Ajout de tests d'intégration contrôleurs POS: `SaleControllerIntegrationTest`, `CustomerControllerIntegrationTest`, `StockControllerIntegrationTest`
